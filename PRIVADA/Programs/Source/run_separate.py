...
def main():
    ...
    processes = [subprocess.Popen(f"docker exec pridaservice sh -c \"python Programs/Source/run_privada.py --compile {COMPILE} --prog {PROG} -N {N} -M {M} --batch-size {BATCH_SIZE} --n-batch-size {DO_BATCH_SIZE} --timeout {CLIENT_TIMEOUT} --run {RUN} --n-threads {N_THREADS} --log-dir {LOG_DIR}\"", shell=True), 
